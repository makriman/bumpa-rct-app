...
| Production infrastructure baseline | `production-baseline-disabled` | Revision `1929771abe932dfd44aad6763e1f5caff19fa833` is live on five TLS-enabled sslip.io hosts. It runs exactly Caddy, web, API, Postgres and Redis with providers disabled; workers/scheduler/Hermes are absent. | Bind the hardened candidate to its final SHA, publish and scan all five exact image digests, deploy it, then record its smoke and backup evidence. |
...
| Droplet and domains | `partial` | The key is authorized for root and the non-root deploy user; Ubuntu 24.04, Docker, UFW, fail2ban, unattended upgrades and swap are verified. Five temporary sslip.io hosts resolve, have valid TLS and route the provider-disabled baseline. | Deploy and verify the hardened candidate. The planned `bumpabestie.com` remains unregistered and is not launch-ready. |
...
| Plan lines / section | Status | Repository evidence | Open requirement / exit evidence |
|---|---|---|---|
| 1982–2169, 20. Compose/Caddy | `partial` | Rendered contracts publish only Caddy; database/cache networks are internal; health checks, volumes and security settings exist. The prior five-service production baseline is inspected and live. | Hermes is absent. Production excludes worker/scheduler. Render and inspect the exact hardened-candidate environment before promotion. |
| 2170–2262, 21. Images/server exclusion | `implemented-tested` | Digest-pinned multi-stage API/web/Caddy/PostgreSQL/backup Dockerfiles, non-root application/Caddy runtimes, production capability/read-only boundaries and no Codex runtime reference are present. Hardened infrastructure images have zero local fixable critical/high findings. | Bind the final SHA, publish all five `linux/amd64` images and scan each exact registry digest. |
| 2263–2373, 22. DigitalOcean deployment | `partial` | SSH, host hardening, Docker, firewall, deploy user, immutable API/web publication, provider-disabled deployment and five-host temporary DNS/TLS are verified for `1929771abe932dfd44aad6763e1f5caff19fa833`. | Publish/pull/deploy the hardened five-image candidate and record its transcript; final branded DNS is externally blocked. |
| 2374–2410, 23. Backup/restore | `partial` | Restricted-capability custom-format backup creates an atomic format-2 manifest/checksum set. Isolated local restore resets `public`, removes newer-only objects, restores export/reserved-Hermes artifacts and reapplies the restricted app role. | Timer activation is not production-proven. The unit does not load an offsite hook from `.env.production`; implement a reviewed handoff/credential boundary, alert and remote restore. Full Hermes restore waits for Hermes. |
...
| Immutable images | Five API/web/Caddy/PostgreSQL/backup digests tagged `sha-<full-sha>`, provenance/SBOM and exact-digest vulnerability results | Prior API/web baseline published; hardened five-image publication/scan pending |
...
| Host hardening | OS, capacity, firewall, updates, Docker and published-port transcript | Complete for the live baseline; revalidate after hardened deployment |
...
| Production environment | Mode `production`, mock/demo rejected, providers `disabled`, async runtime false, file mode 0600 | Complete for `1929771abe932dfd44aad6763e1f5caff19fa833`; hardened release update pending |
| Migration/start | Migration transcript and healthy Caddy/web/API/Postgres/Redis on immutable images | Complete for `1929771abe932dfd44aad6763e1f5caff19fa833`; hardened five-image deployment pending |
