...
| Plan lines / section | Status | Repository evidence | Open requirement / exit evidence |
|---|---|---|---|
| 1982–2169, 20. Compose/Caddy | `partial` | Rendered contracts and the deployed hardened release publish only Caddy; database/cache networks are internal; exact five-service state, health, versions and zero restarts are verified. | Hermes is absent. Production excludes worker/scheduler until their activation gates pass. |
| 2170–2262, 21. Images/server exclusion | `implemented-tested` | All five `linux/amd64` images for `54bb8e9b29295171d65972e094e508d25a7bc53d` were published by run 29194814472 with provenance/SBOM and passing exact-digest scans. Production consumes those exact index references; non-root/capability/read-only boundaries and no Codex runtime are verified. | Revalidate for each later image revision. |
...
| 2374–2410, 23. Backup/restore | `partial` | Restricted-capability backup `20260712T140353Z` has a verified format-2 manifest/checksum, release and exact image reference; the production timer is enabled. Isolated local restore resets `public`, restores artifacts and reapplies the restricted app role. | The unit does not load an offsite hook from `.env.production`; implement a reviewed handoff/credential boundary, alert and remote restore. Full Hermes restore waits for Hermes. |
...
| 2502–2553, 26. Backend tests | `implemented-tested` | Ruff, formatting, strict mypy, pytest branch coverage and migrations passed for the exact release in main CI 29194621699. | Add the specified provider matrix. |
