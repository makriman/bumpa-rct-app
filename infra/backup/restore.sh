...
if [[ -f "$BACKUP_PATH/hermes-runtime.tar.gz" && -d /source/hermes-runtime ]]; then
  find /source/hermes-runtime -mindepth 1 -maxdepth 1 -exec rm -rf -- {} +
  tar -C /source/hermes-runtime -xzf "$BACKUP_PATH/hermes-runtime.tar.gz"
elif [[ -f "$BACKUP_PATH/hermes.tar.gz" && -d /source/hermes-runtime ]]; then
  # Format-2 backups used one Hermes archive. Restore that legacy archive into
  # the runtime source of truth; staging can be regenerated by the control plane.
  find /source/hermes-runtime -mindepth 1 -maxdepth 1 -exec rm -rf -- {} +
  tar -C /source/hermes-runtime -xzf "$BACKUP_PATH/hermes.tar.gz"
fi

if [[ -f "$BACKUP_PATH/hermes-staging.tar.gz" && -d /source/hermes-staging ]]; then
  find /source/hermes-staging -mindepth 1 -maxdepth 1 -exec rm -rf -- {} +
  tar -C /source/hermes-staging -xzf "$BACKUP_PATH/hermes-staging.tar.gz"
fi
