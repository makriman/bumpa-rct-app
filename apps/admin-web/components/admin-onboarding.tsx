...
function LockedSteps({ onboarding }: { onboarding: TenantOnboarding }) {
  ...
  return (
    <section className="grid" aria-label="Saved onboarding steps">
      {steps
        .filter(
          ({ id }, index) =>
            id !== "completed" &&
...
export function ResumableOnboardingStart() {
  ...
  const openOnboardings = existing.filter(
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        clearSessionCommandKey(storageKey);
        router.push(`/onboarding/${recovered.id}`);
      } catch {
        setError(messageFor(reason));
      }
    } finally {
    ...
    <AppShell title="Onboard SME">
      ...
      <div className="grid onboarding-layout onboarding-start-layout">
        <OnboardingStartForm
          ...
          onSubmit={submit}
        />
        <OpenOnboardings loading={loading} onboardings={openOnboardings} />
      </div>
    </AppShell>
  );
}

function OnboardingStartForm({
  busy,
  draft,
  onChange,
  onSubmit,
  ...
  busy: boolean;
  draft: TenantDraft;
  onChange: React.Dispatch<React.SetStateAction<TenantDraft>>;
  onSubmit: (event: FormEvent) => Promise<void>;
}) {
  const field = (value: Partial<TenantDraft>) =>
          ...
          <div className="grid grid-2">
            ...
            <div className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  field({ business_category: event.target.value })
                }
              />
            </div>
            <div className="field">
            ...
            <div className="field">
              ...
              <input
                ...
                aria-invalid={
                  draft.timezone.length > 0 && !isIanaTimezone(draft.timezone)
                }
            ...
            <div className="field">
              ...
              <input
                ...
                onChange={(event) =>
                  field({ currency_code: event.target.value.toUpperCase() })
                }
...
type ResumableViewProps = {
  ...
  onBumpaChange: React.Dispatch<React.SetStateAction<BumpaDraft>>;
  onError: (value: string) => void;
  onLoadingChange: (value: boolean) => void;
  onOwnerChange: React.Dispatch<React.SetStateAction<OwnerDraft>>;
  onPhoneLabelChange: (value: string) => void;
  onRefresh: () => Promise<TenantOnboarding>;
  onSyncWindowChange: React.Dispatch<React.SetStateAction<SyncWindow>>;
  owner: OwnerDraft;
  phoneLabel: string;
  syncWindow: SyncWindow;
};

function ResumableOnboardingView(props: ResumableViewProps) {
  const { error, loading, onboarding, onError, onLoadingChange, onRefresh } =
    props;
  if (loading) {
    return (
      <AppShell title="Onboard SME">
  ...
  return (
    <AppShell title="Onboard SME">
      ...
      <div className="grid onboarding-layout onboarding-progress-layout">
        ...
        <OnboardingCurrentStep {...props} onboarding={onboarding} />
      </div>
    </AppShell>
  );
}

function OnboardingProgress({ onboarding }: { onboarding: TenantOnboarding }) {
  const currentIndex = stepIndex(onboarding.current_step);
  return (
    <div className="card card-pad" style={{ marginBottom: 20 }}>
      <div className="timeline" aria-label="Onboarding progress">
        {steps.map((step, index) => {
          const complete =
            index < currentIndex || onboarding.status === "completed";
          const current = step.id === onboarding.current_step;
          return (
            <div
            ...
            >
              <strong style={{ color: current ? "var(--forest)" : undefined }}>
                {step.label}
              </strong>
              <p>
                {complete
...
function OnboardingCurrentStep(
  props: ResumableViewProps & { onboarding: TenantOnboarding },
) {
  const { onboarding, busy } = props;
  ...
  return (
    <Card padded>
      ...
      {onboarding.current_step === "owner" && <OwnerStep {...props} />}
      {onboarding.current_step === "phone" && <PhoneStep {...props} />}
      {onboarding.current_step === "bumpa" && <BumpaStep {...props} />}
      {onboarding.current_step === "initial_sync" && (
        <InitialSyncStep {...props} />
      )}
      {onboarding.current_step === "hermes" && (
        <HermesStep busy={busy} command={props.command} />
      )}
      {onboarding.current_step === "review" && (
        <ReviewStep
          busy={busy}
      ...
      {onboarding.current_step === "completed" && (
        <CompletedStep onboarding={onboarding} />
      )}
    </Card>
  );
}
...
function OwnerStep({
  ...
  const update = (value: Partial<OwnerDraft>) =>
    onOwnerChange((current) => ({ ...current, ...value }));
  return (
    <form
      onSubmit={(event) => {
        event.preventDefault();
        ...
        <div className="grid grid-2">
          ...
          <div className="field">
            ...
            <input
              ...
              onChange={(event) => update({ phone_e164: event.target.value })}
            />
          </div>
          <div className="field">
  ...
  return (
    ...
    >
      ...
      <div className="alert alert-info">
        This maps the owner&apos;s approved WhatsApp number to this tenant.
        Platform-admin roles remain independent.
      </div>
      <button
        className="button button-primary"
        type="submit"
...
function BumpaStep({
  ...
  const update = (value: Partial<BumpaDraft>) =>
    onBumpaChange((current) => ({ ...current, ...value }));
  return (
    <form
      onSubmit={(event) => {
        event.preventDefault();
        ...
        <div className="grid grid-2">
          <div className="field">
            ...
            <span className="field-help" id="bumpa-key-help">
              Write only. It is cleared after every response.
            </span>
          </div>
          <div className="field">
            ...
            <select
              ...
              onChange={(event) =>
                update({
                  scope_type: event.target.value as BumpaDraft["scope_type"],
                })
              }
            >
          ...
          <div className="field">
            ...
            <input
              ...
              onChange={(event) =>
                update({ store_timezone: event.target.value })
              }
              aria-invalid={
            ...
            <span className="field-help" id="bumpa-timezone-help">
              IANA name used for local-day reporting.
            </span>
          </div>
          <div className="field">
            ...
            <input
              ...
              onChange={(event) =>
                update({ store_currency: event.target.value.toUpperCase() })
              }
...
function InitialSyncStep({
  ...
  onSyncWindowChange,
  syncWindow,
}: ResumableViewProps & { onboarding: TenantOnboarding }) {
  const sync = onboarding.initial_sync;
  const succeeded =
    sync?.job_status === "succeeded" &&
    ((sync.sync_status === "success" &&
      sync.completion_quality === "complete") ||
      (sync.sync_status === "partial" &&
        sync.completion_quality === "accepted_partial" &&
        sync.orders_availability === "available"));
  const terminal = sync ? terminalJobStatuses.has(sync.job_status) : false;
  if (!sync) {
    return (
      <div aria-live="polite" aria-busy={busy}>
        <p style={{ color: "var(--ink-soft)", lineHeight: 1.6 }}>
          Queue the first strict Bumpa import. Activation remains locked until
          the run succeeds and is validated.
        </p>
        <div className="grid grid-2">
          <div className="field">
  ...
  return (
    <div aria-live="polite" aria-busy={busy}>
      {[
        ["Queue job", titleCase(sync.job_status)],
        ["Sync run", titleCase(sync.sync_status)],
        ["Completion quality", titleCase(sync.completion_quality)],
        ["Requested window", `${sync.requested_from} to ${sync.requested_to}`],
        [
          "Orders imported",
          `${sync.orders_count ?? "Unavailable"} · ${titleCase(sync.orders_availability)}`,
        ],
      ].map(([label, value]) => (
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        <button
          ...
          disabled={busy}
          onClick={() =>
            void onRefresh().catch((reason) => onError(messageFor(reason)))
          }
        >
          Refresh sync status
        </button>
          ...
          <button
            ...
            onClick={() =>
              void command("initial_sync", "initial-sync", {
                date_from: sync.requested_from,
                date_to: sync.requested_to,
              })
            }
          >
...
function HermesStep({
  ...
  return (
    <div aria-busy={busy}>
      <p style={{ color: "var(--ink-soft)", lineHeight: 1.6 }}>
        Provision an isolated Hermes profile after the initial Bumpa data load.
        Credentials and runtime addresses never reach this browser.
      </p>
      <button
        className="button button-primary"
        type="button"
        ...
        aria-busy={busy}
        onClick={() =>
          void command("hermes", "hermes", { confirmation: "provision" })
        }
      >
  ...
  return (
    <div>
      <div className="alert alert-success">
        All required resources report ready. Activation performs one final
        server-side reconciliation and records the completion audit.
      </div>
      ...
      <button
        ...
        aria-busy={busy}
        onClick={() =>
          void command("review", "complete", { confirmation: "activate" })
        }
      >
...
function CompletedStep({ onboarding }: { onboarding: TenantOnboarding }) {
  return (
    <div>
      ...
      <Link
        className="button button-primary"
        href={`/tenants/${onboarding.tenant_id}`}
      >
        Open tenant
      </Link>
    </div>
