...
export function AppShell({
  ...
  const canManagePlatformAccess = Boolean(
    session?.platform_roles.includes("superadmin"),
  );
  const navigation = surfaceNav.map((group) => ({
    ...group,
    items: group.items.filter(
      (item) => item.href !== "/administrators" || canManagePlatformAccess,
    ),
  }));
  const displayName = session?.user.name ?? "Platform operator";
  const displayRole = session?.platform_roles.includes("superadmin")
    ? "Platform superadmin"
    : "Platform operator";
